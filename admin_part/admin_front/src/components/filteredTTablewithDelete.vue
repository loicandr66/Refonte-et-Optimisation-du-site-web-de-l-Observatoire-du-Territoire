<template>
  <div class="container">
    <h1>Données dans la table plan_urbanisme</h1>
    ...
    <div class="filters">
      ...
      <div class="add">
        <router-link to="/admin/plan/add"><img src="./icons//ajout_icon.png" alt="Ajouter" width="30" height="30"></router-link>
      </div>
    </div>
    ...
    <table v-if="filteredItems.length > 0" border="1">
      ...
      <tbody>
        <tr v-for="(item, i) in filteredItems" :key="i">
          ...
          <td>
            <button class="modify" @click="editRow(item.id_plan)">Modifier</button>
...
export default {
  ...
  methods: {
    ...
    async editRow(id_plan){
      console.log("ID reçu: ", id_plan);
      this.$router.push({name:'planEditForm', params:{id_plan}})
    },
    async deleteRow(id_plan) {
      if (!confirm("Voulez-vous vraiment supprimer cette ligne ?")) return;
      try {
        const res = await fetch(`http://localhost:3000/api/plans/delete/${id_plan}`, {
          method: "DELETE",
        });
