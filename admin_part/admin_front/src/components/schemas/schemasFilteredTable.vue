<template>
  <div class="container">
    <h1>Données dans la table schemas_reg</h1>
    ...
    <div class="filters">
      ...
      <div class="add">
        <router-link to="/admin/schema/add"><img src="../icons/ajout_icon.png" alt="Ajouter" width="30" height="30"></router-link>
      </div>
    </div>
    ...
    <table v-if="filteredItems.length > 0" border="1">
      ...
      <tbody>
        <tr v-for="(item, i) in filteredItems" :key="i">
          ...
          <td>
            <button class="modify" @click="editRow(item.id_schema)">Modifier</button>
            <button class="delete" @click="deleteRow(item.id_schema)">Supprimer</button>
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  methods: {
    ...
    async editRow(id_schema){
      console.log("ID reçu: ", id_schema);
      this.$router.push({name:'schemaEditForm', params:{id_schema}})
    },
    async deleteRow(id_schema) {
      if (!confirm("Voulez-vous vraiment supprimer cette ligne ?")) return;
      try {
        const res = await apiFetch(`/schemas/delete/${id_schema}`, {
          method: "DELETE",
        });
        
        this.items = this.items.filter((item) => item.id_schema !== id_schema);
      } catch (err) {
