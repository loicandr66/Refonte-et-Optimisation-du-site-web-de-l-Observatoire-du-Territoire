<template>
  <div class="container">
    ...
    <div class="filters">
      ...
      <div class="add">
        <router-link to="/admin/procedures/add"><img src="../icons/ajout_icon.png" alt="Ajouter" width="30" height="30"></router-link>
      </div>
    </div>
    ...
    <table v-if="filteredItems.length > 0" border="1">
      ...
      <tbody>
        <tr v-for="(item, i) in filteredItems" :key="i">
          ...
          <td>
            <button class="modify" @click="editRow(item.id_procedure)">Modifier</button>
            <button class="delete" @click="deleteRow(item.id_procedure)">Supprimer</button>
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  methods: {
    ...
    async editRow(id_procedure){
      console.log("ID reçu: ", id_procedure);
      this.$router.push({name:'procEditForm', params:{id_procedure}})
    },
    async deleteRow(id_stats) {
      if (!confirm("Voulez-vous vraiment supprimer cette ligne ?")) return;
      try {
        const res = await apiFetch(`/procedures/delete/${id_stats}`, {
          method: "DELETE",
        });
        
        this.items = this.items.filter((item) => item.id_procedure !== id_procedure);
      } catch (err) {
