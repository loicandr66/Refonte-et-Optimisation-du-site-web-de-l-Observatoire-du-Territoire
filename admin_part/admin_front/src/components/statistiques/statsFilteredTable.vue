<template>
  <div class="container">
    ...
    <div class="filters">
      ...
      <div class="add">
        <router-link to="/admin/stat/add"><img src="../icons/ajout_icon.png" alt="Ajouter" width="30" height="30"></router-link>
      </div>
    </div>
    ...
    <table v-if="filteredItems.length > 0" border="1">
      ...
      <tbody>
        <tr v-for="(item, i) in filteredItems" :key="i">
          ...
          <td>
            <button class="modify" @click="editRow(item.id_stats)">Modifier</button>
            <button class="delete" @click="deleteRow(item.id_stats)">Supprimer</button>
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  methods: {
    ...
    async editRow(id_stats){
      console.log("ID reçu: ", id_stats);
      this.$router.push({name:'statEditForm', params:{id_stats}})
    },
    async deleteRow(id_stats) {
      if (!confirm("Voulez-vous vraiment supprimer cette ligne ?")) return;
      try {
        const res = await apiFetch(`/statistiques/delete/${id_stats}`, {
          method: "DELETE",
        });
        
        this.items = this.items.filter((item) => item.id_stats !== id_stats);
      } catch (err) {
